...
const MaterialRequestsReviewBanner = ({ requestSource = "" }) => {
  ...
  const savePurchaseLine = async (request, item, index) => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.message || "Failed to update purchase tracking.");
      }
  ...
  const fulfillLine = async () => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.message || "Failed to issue stock.");
      }
      const updatedRequest = payload?.request;
      if (updatedRequest?._id) {
        setRequests((previous) =>
  ...
  return (
    <>
      ...
      {confirmFulfillLine ? (
        <div
          className="material-issue-dialog-backdrop"
          role="presentation"
        ...
        >
          ...
          >
            <div>
              <span className="material-issue-dialog-kicker">Issue Stock</span>
            ...
            <div className="material-issue-dialog-summary">
              <span>
            ...
            <label className="material-issue-dialog-field">
            ...
            <div className="material-issue-dialog-actions">
              <button
                type="button"
                className="secondary"
                onClick={closeFulfillLine}
                    ...
                    return (
                      ...
                      >
                        ...
                        {shouldShowPurchaseTracker(item) ? (
                          <div className="material-review-purchase-tracker">
                            <div className="material-review-purchase-title">
                            ...
                            <div className="material-review-purchase-grid">
                              <label>
                                <span>Status</span>
                                <select
                            ...
                            <button
                              type="button"
                              className="material-review-purchase-save"
                              onClick={() =>
                                savePurchaseLine(detailRequest, item, index)
                              }
                              disabled={
                  ...
                  <article className="material-review-card" key={request._id}>
                    ...
                    <div className="material-review-line-items">
                      {getRequestItems(request).map((item, index) => {
                        ...
                        >
                          <span>{index + 1}</span>
                          <div className="material-review-line-item-main">
                          ...
                          {stockLabel ? (
                            <small className="material-review-stock-line">
                              {stockLabel}
                            </small>
                          ) : null}
                          {issuedQty > 0 ? (
                            <small className="material-review-issued-line">
                              Issued: {formatQty(issuedQty)}
                              {item.unit ? ` ${item.unit}` : ""}
                            </small>
                          ) : null}
                          {quantityToOrder > 0 ? (
                            <small className="material-review-order-line">
                              To order: {formatQty(quantityToOrder)}
                              {item.unit ? ` ${item.unit}` : ""}
                            </small>
                          ) : null}
                          ...
                          {canIssue ? (
                            ...
                            <small className="material-review-issued-note">
